...
class TimingAndScoring extends Plugins
{
    ...
    public function onSplit(Struct $SLP)
    {
        ...
        if (!isset($this->Splits[$SLP->PLID][$sSplit]) OR $SLP->$sTime < $this->Splits[$SLP->PLID][$sSplit]) {    // Personal Best
            $sColor = '^2'; // Green
            $this->Splits[$SLP->PLID][$sSplit] = $SLP->$sTime;
        }
        if (!isset($this->Splits[$sSplit]) OR $SLP->$sTime < $this->Splits[$sSplit]) {    // Overall Best
            $sColor = '^5'; // Purple
